...
holding export options."
  ...
  (concat
   ...
   (let ((email (if (org-string-nw-p anon)
     ...
     (format " \\href{mailto:%s}{\\includegraphics[height=1.5em]{resources/email}}" email))
   ;; github
   (let ((github (if (org-string-nw-p anon)
                     "anon"
                   (org-export-data (plist-get info :github) info))))
     (when (org-string-nw-p github)
       (format " \\href{https://github.com/%s/}{\\includegraphics[height=1.5em]{resources/github}}" github)))
   ;; linkedin
   (let ((linkedin (if (org-string-nw-p anon)
                     "anon"
                   (org-export-data (plist-get info :linkedin) info))))
     (when (org-string-nw-p linkedin)
       (format " \\href{https://linkedin.com/in/%s/}{\\includegraphics[height=1.5em]{resources/linkedin}}" linkedin)))
   ;; twitter
   (let ((twitter (if (org-string-nw-p anon)
                     "anon"
                   (org-export-data (plist-get info :twitter) info))))
     (when (org-string-nw-p twitter)
       (format " \\href{https://twitter.com/%s/}{\\includegraphics[height=1.5em]{resources/twitter}}" twitter)))
   (format "\n\\end{center}\n\n")
